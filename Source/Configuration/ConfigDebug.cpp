...
void ShowDebugValue(int value, std::string valueName)
{
    if(ConfigDebug::debugParams)
    {
        ...
        stream << "Value" << value << "\n";
        Dialog::ShowInfoBox(stream.str(),"[CortiCustomCrit] Debug Configuration Data");
    }
}
...
void Load()
{
    ...
    if(configMap.count("DebugHeroIdVariable"))
    {
        ...
        if(temp<0)
        {
            Dialog::ShowInfoBox(temp, "Invalid DebugHeroIdVariable");
    ...
    if(configMap.count("DebugRawCritVariable"))
    {
        ...
        if(temp<0)
        {
            Dialog::ShowInfoBox(temp, "Invalid DebugRawCritVariable");
    ...
    if(configMap.count("DebugTargetLevelVariable"))
    {
        ...
        if(temp<0)
        {
            Dialog::ShowInfoBox(temp, "Invalid DebugTargetLevelVariable");
    ...
    if(configMap.count("DebugAdeptedCritVariable"))
    {
        ...
        if(temp<0)
        {
            ...
            Dialog::ShowInfoBox(temp, "Invalid DebugAdeptedCritVariable");
        }
        else if(temp>0)
        {
    ...
    if(configMap.count("DebugAppliedAttackBonusVariable"))
    {
        ...
        if(temp<0)
        {
            Dialog::ShowInfoBox(temp, "Invalid DebugAppliedAttackBonusVariable");
